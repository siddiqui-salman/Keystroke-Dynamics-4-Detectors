...
def euclideandetectornorm(data, sample, users):
...
def visualroc(fpr, tpr, n, color, sample_size):
    ...
    for i in n:
        ...
        plt.title('Euclidean Detector (Normalized) :Receiver Operating Characteristic for '+'{}'.format(len(n))+' users  (N= {})'.format(sample_size))
...
def visualdet(fpr, ipr, n, t,sample_size,color):
    ...
    for i in n:
        ...
        plt.title('Euclidean Detector (Normalized) : Detection Error Tradeoff for' + '{}'.format(len(n)) + ' users (N= {})'.format(sample_size))
        plt.legend(loc="lower right")
    plt.show()
...
def evaluateScore(template, score_list,users):
    ...
    for i in range(len(users)):
        for j in range(len(score_list[i])):
            score.append(distance.euclidean(score_list[i].values[j], template[i]))
