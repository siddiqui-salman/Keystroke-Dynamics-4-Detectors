...
def manhattandetectorscaled(data, sample, users):
    ...
    for i in range(0, train.shape[0], sample):
        ...
        mad.append(train.iloc[i:i+sample].mad())
    ...
    impostor_score = evaluateScore(mean_vector, impostor_user, users,mad)
    genuine_score = evaluateScore(mean_vector, genuine_user, users,mad)
...
def visualroc(fpr, tpr, n, color, sample_size):
    ...
    for i in n:
        ...
        plt.title('Manhattan Detector (Scaled) Receiver Operating Characteristic for '+'{}'.format(len(n))+' users  (N= {})'.format(sample_size))
...
def visualdet(fpr, ipr, n, t,sample_size,color):
    ...
    for i in n:
        ...
        plt.title('Manhattan Detector (Scaled) Detection Error Tradeoff for'+'{}'.format(len(n))+' users (N= {})'.format(sample_size))
        plt.legend(loc="lower right")
    plt.show()
...
def evaluateScore(template, score_list,users,mad):
    score = []
    total = []
    for i in range(len(users)):
        score.append(abs(template[i] - score_list[i])/mad[i])
