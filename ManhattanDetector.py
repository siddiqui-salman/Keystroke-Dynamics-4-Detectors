...
def manhattandetector(data, sample, users):
...
def visualroc(fpr, tpr, n, color, sample_size):
    ...
    for i in n:
        ...
        plt.title('Manhattan Detector : Receiver Operating Characteristic for '+'{}'.format(len(n))+' users  (N= {})'.format(sample_size))
...
def visualdet(fpr, ipr, n, t,sample_size,color):
    ...
    for i in n:
        ...
        plt.title(' Manhattan Detector: Detection Error Tradeoff for' + '{}'.format(len(n)) + ' users (N= {})'.format(sample_size))
        plt.legend(loc="lower right")
    plt.show()
